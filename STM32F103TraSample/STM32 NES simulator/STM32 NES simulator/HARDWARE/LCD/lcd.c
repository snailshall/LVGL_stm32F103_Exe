...
static u16 READ_BUS()
{
    u16 idr, tmp;
    idr = GPIOD->IDR;
    tmp |= ((idr&0xC000)>>14)|((idr&0x0003)<<2)|((idr&0x0700)<<5);
    idr = GPIOE->IDR;
    tmp |= ((idr&0xFF80)>>3);
    return tmp;
}

static void SET_BUS_IN()
{
...
//data:寄存器值
static void LCD_WR_DATAX(u16 data)
{
    LCD_RS_SET;
    LCD_CS_CLR;
    WRITE_BUS(data);
...
void LCD_Init(void)
{
    ...
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOE|RCC_APB2Periph_GPIOD|RCC_APB2Periph_AFIO, ENABLE); //使能PORTB,C时钟和AFIO时钟
    ...
    // LCD 控制端口
    GPIO_InitStructure.GPIO_Pin = GPIO_Pin_4|GPIO_Pin_7|GPIO_Pin_11|GPIO_Pin_5;
    GPIO_Init(GPIOD, &GPIO_InitStructure);
    GPIO_SetBits(GPIOD, GPIO_Pin_4|GPIO_Pin_7|GPIO_Pin_11|GPIO_Pin_5);
    ...
    GPIO_Init(GPIOD, &GPIO_InitStructure);
    GPIO_InitStructure.GPIO_Pin = ((uint16_t)0xFF80);
    ...
    delay_ms(50); // delay 50 ms
    lcddev.id = 0x9341; //LCD_ReadReg(0x0000);

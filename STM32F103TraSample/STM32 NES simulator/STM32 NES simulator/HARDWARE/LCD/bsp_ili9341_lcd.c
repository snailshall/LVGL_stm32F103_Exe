...
// 0竖屏，游戏方向与屏幕上的“野火”二字方向相反
// 6竖屏，游戏方向与屏幕上的“野火”二字方向一致
// 3横屏，游戏方向的头顶是开发板JTAG接口
// 5横屏，游戏方向的头顶是开发板GPIO接口
...
static void ILI9341_FSMC_Config ( void )
{
    ...
    ReadWriteTimingStructure.FSMC_AddressSetupTime       = 0x01;   //地址建立时间
    //数据保持时间（DATAST）+ 1个HCLK = 5/72M=70ns
    ReadWriteTimingStructure.FSMC_DataSetupTime          = 0x04;   //数据建立时间
    //选择控制的模式
    //模式B,异步NOR FLASH模式，与ILI9341的8080时序匹配
    ReadWriteTimingStructure.FSMC_AccessMode             = FSMC_AccessMode_B;

    /*以下配置与模式B无关*/
    //地址保持时间（ADDHLD）模式A未用到
    ReadWriteTimingStructure.FSMC_AddressHoldTime        = 0x00;   //地址保持时间
    ...
    FSMC_NORSRAMInitStructure.FSMC_WriteTimingStruct     = &ReadWriteTimingStructure;
...
static void ILI9341_REG_Config ( void )
{
    ...
    ILI9341_Write_Cmd ( CMD_SetAxisX );
    ILI9341_Write_Data ( 0x00 );
    ILI9341_Write_Data ( 0x00 );
    ILI9341_Write_Data ( 0x00 );
    ...
    DEBUG_DELAY ();
    ILI9341_Write_Cmd ( CMD_SetAxisY );
    ILI9341_Write_Data ( 0x00 );
    ILI9341_Write_Data ( 0x00 );
    ILI9341_Write_Data ( 0x01 );
...
void ILI9341_Init ( void )
{
    ...
    //清空屏幕
    ILI9341_Clear(0, 0, LCD_X_LENGTH-1, LCD_Y_LENGTH-1);
}
...
void ILI9341_Clear ( uint16_t usX, uint16_t usY, uint16_t usWidth, uint16_t usHeight )
{
    ...
    ILI9341_FillColor ( usWidth * usHeight, WHITE );
}
...
void ILI9341_DrawRectangle ( uint16_t usX_Start, uint16_t usY_Start, uint16_t usWidth, uint16_t usHeight, uint8_t ucFilled )
{
    if ( ucFilled )
    {
        ...
        ILI9341_FillColor ( usWidth * usHeight, BLACK );
    }
    else
    {

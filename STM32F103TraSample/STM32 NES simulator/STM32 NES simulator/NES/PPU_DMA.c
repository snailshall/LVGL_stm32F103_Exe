...
/*显示相关*/
#define SCANLINE_SIZE  (8 + 256 + 8)
uint8   SpriteHitFlag, PPU_Latch_Flag;  //sprite #0 显示碰撞点所在扫描行号，背景位移￥2005写入标志
int     PPU_scanline;                   //当前扫描行
uint16  Buffer_scanline[SCANLINE_SIZE];   //行显示缓存，上下标越界最大为7，显示区7~263 0~7 263~270为防止溢出区
...
const uint16 NES_Color_Palette[64] =
{
    ...
    0xFFFF,0x3DFF,0x5CBF,0xA45F,0xF3DF,0xFBB6,0xFBAC,0xFCC7,0xF5E7,0x8682,0x4EE9,0x5FD3,0x075B,0x0000,0x0000,0x0000,
               ...
               uint8  ScreenMirrorType ) /* 屏幕镜像类型*/
{
    ...
    DMA_InitBlock.DMA_MemoryBaseAddr = (uint32_t)(&Buffer_scanline[16]);
    DMA_InitBlock.DMA_DIR = DMA_DIR_PeripheralDST;
    DMA_InitBlock.DMA_BufferSize = (uint32_t)240;
...
void NES_RenderLine(int y_axes)
{
    ...
    {
        /*清空显示缓存，在此设置底背景色（待确定）*/
        for(i = 0; i < SCANLINE_SIZE; i++) //显示区 7~263 0~7 263~270 为防止溢出区
        {
    ...
    {
        for(i=0; i<SCANLINE_SIZE; i++)
        {
            Buffer_scanline[i] = BLACK; //清空显示缓存,黑屏
        }
    }
    /*完成扫描，将行显示缓存写入LCD*/
    NES_LCD_RenderLine(y_axes);
}
//-------------------------------------------------------------------------------

// 为加速渲染，使用宏重写‘写命令’和‘写数据’操作。
...
// 为加速渲染，使用宏重写渲染前的准备指令
#define LCD_RENDER_PREPARE(__x1__, __y1__, __x2__, __y2__) \
{ \
    LCD_WRITE_CMD(CMD_SetAxisX); \
    LCD_WRITE_DATA((__x1__) >> 8); \
    LCD_WRITE_DATA((__x1__) & 0xff); \
    LCD_WRITE_DATA((__x2__) >> 8); \
    LCD_WRITE_DATA((__x2__) & 0xff); \
 \
    LCD_WRITE_CMD(CMD_SetAxisY); \
...
// 渲染游戏某一帧的某一行。即模拟PPU将一行缓存写入LCD
void NES_LCD_RenderLine(int y_axes)
{
#if 0
    int32_t i;
    y_axes += 40;
    LCD_RENDER_PREPARE(0, y_axes, 239, y_axes);
    for (i = 16; i < 256; i++)
    {
        LCD_WRITE_DATA(Buffer_scanline[i]);
    }
#else
    ...
    DMA_SetCurrDataCounter(DMA1_Channel5, 240);

    DMA_ClearFlag(DMA1_FLAG_GL5);
    DMA_ITConfig(DMA1_Channel5, DMA_IT_TC, ENABLE);
    DMA_Cmd(DMA1_Channel5, ENABLE);

//    while (!DMA_GetITStatus(DMA1_IT_TC5)) ; // 等待DMA拷贝完成
...
// 清空背景色
void NES_LCD_Clear(uint16 color)
{
    uint16_t i, len;
    LCD_RENDER_PREPARE(0, 0, LCD_X_LENGTH-1, LCD_Y_LENGTH-1);
    len = LCD_X_LENGTH * LCD_Y_LENGTH;
    for (i = 0; i < len; i++)
    {
        LCD_WRITE_DATA(Buffer_scanline[i]);
    }

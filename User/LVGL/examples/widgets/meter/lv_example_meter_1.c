...
void lv_example_meter_1(void)
{
    ...
    lv_meter_set_scale_major_ticks(meter, scale, 10, 4, 7, lv_color_black(), 10);

    lv_meter_indicator_t * indic;
	
		indic = lv_meter_add_arc(meter, scale, 2, lv_palette_main(LV_PALETTE_GREY), 0);
    lv_meter_set_indicator_start_value(meter, indic, 20);
    lv_meter_set_indicator_end_value(meter, indic, 80);
	

    /*Add a blue arc to the start*/
    indic = lv_meter_add_arc(meter, scale, 2, lv_palette_main(LV_PALETTE_GREEN), 0);
    ...
    indic = lv_meter_add_needle_line(meter, scale, 3, lv_palette_main(LV_PALETTE_DEEP_ORANGE), -10);
